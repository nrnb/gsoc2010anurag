...
//                JOptionPane.INFORMATION_MESSAGE);
        ...
        File downloadParentDirectory = new File(FileLocator.downloadDirLocation);
        if (!downloadParentDirectory.exists()) {
            downloadParentDirectory.mkdir();
        }

        File tempDirectory = new File(FileLocator.downloadDirLocation + "/" + ID);
...
//        System.out.println("finally " + task.getActiveThreadCount() + " threads still active");
    ...
    public synchronized void onDownloadFinish(DownloadEvent evt) {
        ...
        File testFile = new File(FileLocator.home_dir + "/" + outputFileName);
        if (testFile.exists()) {
            String newName = null;
            for (int i = 1; testFile.exists(); i++) {
                newName = outputFileName + "_" + i;
                testFile = new File(FileLocator.home_dir + "/" + newName);
            }
...
//        throw new UnsupportedOperationException("Not supported yet.");
        ...
        try {
            outputFile = new File(FileLocator.home_dir + "/" + outputFileName);
            BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(outputFile, true));
            byte buffer[] = new byte[1024];

            for (int i = 0; i < JET_COUNT; i++) {
                File file = new File(FileLocator.downloadDirLocation + "/" + identifier + "/" + identifier + "part" + i);
            ...
            File tempDirectory = new File(FileLocator.downloadDirLocation + "/" + identifier);
